...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Запам'ятовуємо клієнта в базі — потрібно для розсилок і статистики.
    db.upsert_client(
    ...
    # Запам'ятовуємо дані доставки для профілю — наступного разу
    # клієнт побачить їх у /profile і зможе просто повторити.
    db.save_delivery_details(message.from_user.id, name, phone, point)
...
async def cancel_order_execute(callback: CallbackQuery, state: FSMContext):
    ...
    db.set_order_status(order_no, "cancelled")
...
async def admin_confirm_payment(callback: CallbackQuery):
    ...
    db.set_order_status(order_no, "paid")
    lang = await get_client_lang(client_id)
    try:
        await bot.send_message(
            chat_id=client_id,
...
async def admin_send_tracking(message: Message, state: FSMContext):
    ...
    db.set_order_status(order_no, "shipped", tracking_no=tracking)
...
async def export_generate(callback: CallbackQuery):
    ...
    try:
        if kind == "clients":
            rows = db.get_clients_for_export()
            if not rows:
            ...
            rows = db.get_orders_for_export(days=days, status=status)
            if not rows:
    ...
    if action == "add" and len(parts) >= 4:
        ...
        if len(parts) >= 5:
            try:
                ...
                pass
        if db.add_promo(code, percent, max_uses):
    ...
    for i, chat_id in enumerate(ids, 1):
        try:
            ...
            if "blocked" in str(e).lower() or "chat not found" in str(e).lower():
                db.mark_client_blocked(chat_id)
...
async def cmd_profile(message: Message, state: FSMContext):
    ...
    chat_id = message.from_user.id
    profile = db.get_client_profile(chat_id)
    orders = db.get_client_orders(chat_id, limit=5)
...
async def cmd_my_orders(message: Message):
    ...
    orders = db.get_client_orders(message.from_user.id, limit=10)
    if not orders:
