...
def open_shop_keyboard(lang: str) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=t(lang, "open_shop_button"), web_app=WebAppInfo(url=WEBAPP_URL))],
            [
...
async def cmd_promo(message: Message):
    ...
    if action == "add" and len(parts) >= 4:
        ...
        if await db.a_add_promo(code, percent, max_uses):
            ...
            await message.answer(f"✅ Промокод <code>{code}</code> додано — знижка {percent}%{limit_txt}", parse_mode="HTML")
        else:
...
async def cmd_broadcast_text(message: Message, state: FSMContext):
    ...
    if text.strip().lower() in ("скасувати", "cancel", "/cancel"):
        await state.clear()
...
async def cmd_profile(message: Message, state: FSMContext):
    ...
    if not profile:
        await message.answer(t(lang, "profile_empty"), reply_markup=open_shop_keyboard(lang))
        return
