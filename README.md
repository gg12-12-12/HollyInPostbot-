...
4. Перетягніть **чотири** файли:
   - `bot.py`
   - `requirements.txt`
   - `Procfile`
   - `runtime.txt` ← важливо, задає версію Python
...
## Як працює обробка замовлення

**У клієнта:**
1. Оформлює замовлення в Mini App
2. Отримує номер замовлення (напр. `HL-1908-4821`) і реквізити BLIK
3. Оплачує, надсилає скрін

**У вас (менеджера):**
Під кожним замовленням у чаті з'являються дві кнопки:

- **✅ Оплату зараховано** — клієнту миттєво прийде повідомлення
  «Оплату зараховано, замовлення прийнято в роботу» з його номером замовлення

- **📦 Надіслати номер посилки** — бот попросить вас ввести трек-номер InPost.
  Просто напишіть його наступним повідомленням — клієнт отримає номер
  і посилання на відстеження. Щоб відмінити, напишіть `скасувати`.

Клієнт отримує повідомлення його мовою (українська / польська / англійська) —
бот запам'ятовує, якою мовою він користувався в магазині.
...
**Deploy failed, у логах `metadata-generation-failed` або `pydantic-core`**
→ Це означає, що Render узяв надто нову версію Python (3.13/3.14),
для якої бібліотеки ще не мають готових збірок.
Переконайтесь, що у репозиторії лежить файл **`runtime.txt`** із текстом:
```
3.11.9
```
Після його додавання натисніть на Render **Manual Deploy → Deploy latest commit**.
